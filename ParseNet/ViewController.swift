...
class ViewController: UIViewController, VideoCaptureDelegate {
    ...
    override func viewDidLoad() {
        ...
        createNeuralNetwork {
            ...
            self.predict(imageNamed: "wx.jpg")
    ...
    private func createNeuralNetwork(completion: @escaping () -> Void) {
        ...
        DispatchQueue.global().async {
            ...
            self.nn = ParseNet(device: self.device)
    ...
    private func predict(texture: MTLTexture, previewImage: UIImage, bgr: Bool) {
        ...
        var previewImage1 = previewImage//.fixImageOrientation()
        imageView.image = previewImage1
        
        //print(previewImage.imageOrientation.rawValue)
        
        if (previewImage.imageOrientation == .right)
        {
        //previewImage1 = imageRotatedByDegrees(oldImage: previewImage, deg: 90)
            previewImage1 = previewImage.fixImageOrientation()!
        }
        ...
        print(previewImage1.size)
        print(previewImage.size.height)
        print(previewImage1.size.height)
        print(previewImage1.imageOrientation.rawValue)
        button.isEnabled = false
        
        //added by Guigu
        var texture1:MTLTexture!
        do{
        try texture1 = textureLoader.newTexture(with: (previewImage1.cgImage!), options: [
        ...
        // ParseNet.predict() blocks until the GPU is ready, so to prevent the app's
        // UI from being blocked we call that method from a background thread.
        DispatchQueue.global().async {
            let inputImage = self.image(from: texture1)
    ...
    private func show(prediction: [Prediction]) {
        let segImg = UIImage.image(textureRGBA16Float: nn.seg_img.texture)
        let segImg_display = UIImage(cgImage: segImg.cgImage!, scale: 1.0, orientation: UIImageOrientation.up)
        imageLabel.image = segImg_display
        
        //imageLabel.image = UIImage.image(textureRGBA16Float: nn.seg_img.texture)
